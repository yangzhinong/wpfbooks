    ...
    class Program
    {
        ...
        private static void Main(string[] args)
        {
           ...
           IObservable<Tuple<string,int>> networks=  Observable.FromEvent<ExtendedNetworkFoundEventHandler, Tuple<string, int>>(
                                    rxHandler =>  (ssid, strlength) => rxHandler(Tuple.Create(ssid, strlength)),
                                    h => wifiScanner.ExtendedNetworkFound += h,
                                    h => wifiScanner.ExtendedNetworkFound -= h);

            IObservable<Unit> connected = Observable.FromEvent(h => wifiScanner.Connected += h, h => wifiScanner.Connected -= h);



            IEnumerable<string> cities = new[] { "London", "Tel-Aviv", "Tokyo", "Rome", "Madrid" };

            var lookupObservable = cities.ToObservable()
                                         .ToLookup(x => x.Length);
            ...
            lookupObservable.Select(lookup =>
            {
                ...
                foreach (var g in lookup)
                {
                    ...
                    groups.AppendFormat("[key:{0} => {1}", g.Key, g.Count());
